...
impl TuiImageCompositor {
    ...
    pub fn render(
        ...
        for placement in scene.placements() {
            ...
            if visible.x != placement.destination.x || visible.y != placement.destination.y {
                continue;
            }
            ...
            let visible_pixel_size = crop_right_bottom(
                &mut payload,
...
fn crop_right_bottom(
    ...
    let cropped_width = proportional_pixels(pixels.width, visible_width, destination_width);
    let cropped_height = proportional_pixels(pixels.height, visible_height, destination_height);
    ...
    {
        data.extend_from_slice(&row[..cropped_row_bytes.min(row.len())]);
    }
...
mod tests {
    ...
    fn replacement_and_removal_reconcile_stable_keys() {
        ...
        compositor
            ...
            .unwrap();
        assert!(String::from_utf8(replaced).unwrap().contains("a=t"));
