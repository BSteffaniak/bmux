...
mod pipeline {
    ...
    fn sixel_intercept_to_registry_to_delta() {
        ...
        assert_eq!(img.position.row, 5);
        assert_eq!(img.position.col, 10);
    ...
    fn scroll_shifts_image_positions() {
        ...
        // Add an image at row 10.
        let input = b"\x1bPq~\x1b\\";
        let result = interceptor.process(input, (10, 0));
        for event in result.events {
            registry.handle_event(event, 8, 16);
        }
        ...
        // Scroll up by 8 more (image at row 7 with 1 row height → evicted).
    ...
    fn delta_tracks_removals() {
        ...
        for i in 0..3 {
            let input = b"\x1bPq~\x1b\\";
            let mut interceptor = ImageInterceptor::new();
            let result = interceptor.process(input, (i, 0));
            for event in result.events {
                registry.handle_event(event, 8, 16);
            }
        }
    ...
    fn kitty_chunked_transmission() {
        ...
        let result1 = interceptor.process(chunk1, (0, 0));
        ...
        let result2 = interceptor.process(chunk2, (0, 0));
    ...
    fn iterm2_full_pipeline() {
        ...
        let result = interceptor.process(&input, (2, 5));
        assert_eq!(result.events.len(), 1);
        ...
        assert_eq!(img.position.row, 2);
        assert_eq!(img.position.col, 5);
    }
