...
impl<'a> ScrollViewComponent<'a> {
    ...
    pub fn new(
        ...
        Self {
            id: id.into(),
            viewport,
            content_width: u16::try_from(viewport.width).unwrap_or(u16::MAX),
            offset_x: state.horizontal_offset(),
            offset_y: state.vertical_offset(),
            child: Element::new(child),
            follow_bottom: state.follows_bottom(),
            reveal: None,
            reveal_end: false,
            retained_state: None,
        }
    ...
    pub const fn retain_state(mut self, state: &'a std::cell::Cell<ScrollViewState>) -> Self {
        ...
        self.follow_bottom = state.get().follows_bottom();
    ...
    fn effective_state(&self, layout: &LayoutNode) -> ScrollViewState {
        ...
        let maximum_x = ScrollView::max_horizontal_offset(layout);
        let maximum_y = ScrollView::max_vertical_offset(layout);
        let revealed = layout.children.first().and_then(|child| {
            self.reveal
                .as_ref()
                .and_then(|id| child.node.find_logical_rect(id))
        });
        state.horizontal_offset = self.offset_x.min(maximum_x);
        state.follow_bottom = self.follow_bottom && revealed.is_none();
        state.vertical_offset = if state.follow_bottom {
            maximum_y
        } else {
            self.offset_y.min(maximum_y)
        };
...
impl Component for ScrollViewComponent<'_> {
    fn revision(&self) -> ComponentRevision {
        ...
        self.follow_bottom.hash(&mut paint);
    ...
    fn layout(
        ...
        let child = self
            .child
            .layout(Constraints::for_width(self.content_width.into()), cx);
    ...
    fn paint(&self, layout: &LayoutNode, cx: &mut PaintCx<'_, '_>) {
        ...
        let offset = resolved.vertical_offset();
        ...
        cx.with_child_size(0, 0, layout.size, |cx| {
            ...
            cx.with_child(
                ...
                |cx| {
                    cx.with_child_size(0, 0, child.node.size, |cx| {
                        self.child.paint(&child.node, cx);
                    });
                },
            );
        });
    ...
    fn event(&self, event: &Event, layout: &LayoutNode, cx: &mut EventCx<'_>) -> EventOutcome {
        ...
        cx.with_transform(
            ...
            |cx| {
                ...
                if clip.is_empty()
                    || matches!(event, Event::Mouse(mouse) if !clip.contains(mouse.position))
                {
                    return EventOutcome::Ignored;
                }
...
mod tests {
    ...
    #[test]
    fn horizontal_reveal_preserves_vertical_state_and_bottom_follow() {
        let layout = LayoutNode::with_children(
            ...
            vec![ChildLayout::new(
                0,
                0,
                LayoutNode::leaf("content".into(), LogicalSize::new(20, 100)),
            )],
        );
    ...
    #[test]
    fn retained_follow_handles_resize_shrink_and_explicit_navigation() {
        let mut state = ScrollViewState::new();
        state.set_follow_bottom(true);
        let retained = std::cell::Cell::new(state);
        for (text, height, reveal, expected_offset, expected_follow, expected_first) in [
            ("a\nb\nc\nd", 2, false, 2, true, "c"),
            ("a\nb\nc\nd", 3, false, 1, true, "b"),
            ("a", 3, false, 0, true, "a"),
            ("a\nb\nc\nd", 2, false, 2, true, "c"),
            ...
            let component = ScrollViewComponent::new(
                "scroll",
                LogicalSize::new(1, u64::from(height)),
                retained.get(),
                TextBlock::new(text),
            )
            .retain_state(&retained);
            let layout = component.layout(
                Constraints::tight(Size::new(1, height)),
                &mut LayoutCx::new(),
            );
            let component = if reveal {
                component.reveal(layout.children[0].node.id.clone())
            } else {
                component
            };
            let mut buffer = Buffer::empty(Rect::new(0, 0, 1, height));
            let mut frame = Frame::new(&mut buffer);
            component.paint(&layout, &mut PaintCx::new(&mut frame));
            assert_eq!(
                frame
            ...
            assert_eq!(retained.get().vertical_offset(), expected_offset);
            assert_eq!(retained.get().follows_bottom(), expected_follow);
        }
    }

    #[test]
    fn missing_declarative_reveal_preserves_bottom_follow() {
        for end in [false, true] {
            ...
            let component = if end {
                component.reveal_end("missing")
            } else {
                component.reveal("missing")
            };
            let layout =
                component.layout(Constraints::tight(Size::new(1, 2)), &mut LayoutCx::new());
            ...
            assert_eq!(retained.get().vertical_offset(), 1);
            assert!(retained.get().follows_bottom());
        }
    }

    #[test]
    fn component_preserves_bottom_follow_across_appends() {
        for follow in [false, true] {
            let mut state = ScrollViewState::new();
            state.set_follow_bottom(follow);
            let retained = std::cell::Cell::new(state);
            for (text, expected_bottom) in [("a\nb\nc", "b"), ("a\nb\nc\nd", "c")] {
                let component = ScrollViewComponent::new(
                    "scroll",
                    LogicalSize::new(1, 2),
                    state,
                ...
                assert_eq!(
                    frame
                        ...
                        .symbol
                        .as_str(),
                    if follow { expected_bottom } else { "a" }
                );
                assert_eq!(retained.get().follows_bottom(), follow);
            }
        }
        let mut state = ScrollViewState::new();
        let initial = ScrollViewComponent::new(
        ...
        .revision();
        state.set_follow_bottom(true);
        let following = ScrollViewComponent::new(
            "scroll",
            LogicalSize::new(1, 2),
            state,
            TextBlock::new("a\nb\nc"),
        ...
        .revision();
        assert_eq!(initial.layout, following.layout);
        assert_ne!(initial.paint, following.paint);
    }
    ...
    fn declarative_reveal_clamps_overflowing_descendants_on_both_axes() {
        ...
        for end in [false, true] {
            ...
            let resolved = component.effective_state(&layout);
            assert_eq!(resolved.horizontal_offset(), 8);
            assert_eq!(resolved.vertical_offset(), 8);
        }
    }
    ...
    fn scroll_content_events_are_bounded_by_child_and_parent() {
        ...
        impl Component for ClipHandler {
            fn layout(&self, _constraints: Constraints, _cx: &mut LayoutCx) -> LayoutNode {
                LayoutNode::leaf("handler".into(), LogicalSize::new(2, 1))
            }
            fn paint(&self, _layout: &LayoutNode, _cx: &mut PaintCx<'_, '_>) {}
            fn event(
                &self,
                ...
                assert_eq!(cx.clip(), Some(Rect::new(0, 0, 2, 1)));
                EventOutcome::Handled
            }
        }
        let mut state = ScrollViewState::new();
        ...
        for (x, y, expected) in [
            (1, 0, EventOutcome::Handled),
            (2, 0, EventOutcome::Ignored),
            (0, 1, EventOutcome::Ignored),
        ] {
            let event = Event::Mouse(MouseEvent::new(
                MouseEventKind::Down(bmux_tui::event::MouseButton::Left),
                Point::new(x, y),
            ));
            assert_eq!(component.event(&event, &layout, &mut cx), expected);
        }
        let mut cx = EventCx::with_clip(&layout, Rect::new(0, 1, 4, 2));
        assert_eq!(
            component.event(&Event::Paste("input".into()), &layout, &mut cx),
            EventOutcome::Ignored
        );
    }

    #[test]
    fn scroll_content_paint_is_bounded_by_resolved_child_size() {
        struct Fill;
        impl Component for Fill {
            fn layout(&self, _constraints: Constraints, _cx: &mut LayoutCx) -> LayoutNode {
                LayoutNode::leaf("fill".into(), LogicalSize::new(2, 1))
            }
            fn paint(&self, _layout: &LayoutNode, cx: &mut PaintCx<'_, '_>) {
    ...
    #[test]
    fn tall_scroll_view_paints_inside_deep_parent_viewport() {
        let text = bmux_tui::text::Text::from_lines(
            (0..70_003)
                .map(|row| bmux_tui::text::Line::raw(format!("{row:05}")))
                .collect::<Vec<_>>(),
        );
        let component = ScrollViewComponent::new(
            "scroll",
            LogicalSize::new(5, 70_003),
            ScrollViewState::new(),
            TextBlock::new(text),
        );
        ...
        PaintCx::new(&mut frame).with_child(
            0,
            -70_000,
            bmux_tui::paint::LocalRect::new(0, 70_000, 5, 3),
    ...
    #[test]
    fn translated_scroll_view_events_use_terminal_space_clip() {
        let mut state = ScrollViewState::new();
        ...
        let root = LayoutNode::with_children(
            "root".into(),
            LogicalSize::new(20, 20),
            vec![ChildLayout::new(4, 8, layout)],
        );
        let child = &root.children[0];
        let mut cx = EventCx::with_clip(&root, Rect::new(0, 0, 20, 20));
        for (x, y, expected) in [
            (5, 8, EventOutcome::Handled),
            (5, 9, EventOutcome::Handled),
            (5, 10, EventOutcome::Ignored),
            (3, 8, EventOutcome::Ignored),
            (1, 1, EventOutcome::Ignored),
        ] {
            let event = Event::Mouse(MouseEvent::new(
                MouseEventKind::Down(bmux_tui::event::MouseButton::Left),
                Point::new(x, y),
            ...
            assert_eq!(
                cx.with_child(child, |cx| component.event(&event, &child.node, cx)),
                expected
            );
        }
    }
    ...
    fn reconcile_clamps_both_logical_axes_without_terminal_saturation() {
        let content = LayoutNode::leaf(
            ...
            LogicalSize::new(u16::MAX.into(), 100_000),
        );
        let layout = LayoutNode::with_children(
            LayoutId::new("scroll"),
