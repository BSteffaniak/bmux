...
pub async fn run_session_attach_with_client(
    ...
    // ── First-tab-on-attach bootstrap ───────────────────────────────
    //
    // On a fresh server (no contexts yet) or an existing server whose
    // snapshot restored only orphan sessions without bound contexts,
    // `attach_info.context_id` is None. Without this bootstrap the user
    // would land in an empty view and have to press `c` to create a
    // tab. Dispatching the same plugin command the `c` keybinding
    // triggers creates a default context atomically via contexts-plugin
    // and fires SessionActiveContextChanged so the attach view
    // retargets.
    //
    // Guard:
    //   * `follow_target_id.is_none()` — in follow mode the leader
    //     drives context selection.
    //   * `attach_info.context_id.is_none()` — grant didn't bind a
    //     context.
    //   * server-wide context list is empty — differentiates the
    //     "fresh server" case from "this session is orphaned but other
    //     contexts exist" (where the user should pick from existing).
    let mut attach_info = attach_info;
    if follow_target_id.is_none() && attach_info.context_id.is_none() {
        let contexts = typed_list_contexts_attach(&mut client)
            .await
            .unwrap_or_default();
        if contexts.is_empty() {
            tracing::debug!(
                target: "bmux_cli::runtime::attach",
                session_id = %attach_info.session_id,
                "attach bootstrap: no contexts, dispatching bmux.windows:new-window",
            );
            match run_plugin_keybinding_command(
                ...
                Ok(execution) if execution.status == 0 => {
                    // Pick up the freshly-created context and rebind
                    // attach_info to it so the rest of startup sees a
                    // proper (session, context) pair.
                    match typed_current_context_attach(&mut client).await {
                        Ok(Some(ctx)) => match open_attach_for_context(&mut client, ctx.id).await {
                            Ok(info) => {
                                tracing::info!(
                                    target: "bmux_cli::runtime::attach",
                                    context_id = %ctx.id,
                                    session_id = %info.session_id,
                                    "attach bootstrap: bound to new context",
                                );
                                attach_info = info;
                            }
                            Err(err) => {
                                tracing::warn!(
                                    target: "bmux_cli::runtime::attach",
                                    error = %err,
                                    "attach bootstrap: open_attach_for_context failed, continuing with original grant",
                                );
                            }
                        },
                        Ok(None) => {
                            tracing::warn!(
                                target: "bmux_cli::runtime::attach",
                                "attach bootstrap: new-window completed but current-context is still None",
                            );
                        }
                        Err(err) => {
                            tracing::warn!(
                                target: "bmux_cli::runtime::attach",
                                error = %err,
                                "attach bootstrap: current-context query failed",
                            );
                        }
                    }
                }
                Ok(execution) => {
                    tracing::warn!(
                        ...
                        error = execution.outcome.error_message.as_deref().unwrap_or(""),
                        "attach bootstrap: new-window returned nonzero status",
                    );
                }
                Err(err) => {
                    tracing::warn!(
                        target: "bmux_cli::runtime::attach",
                        error = %err,
                        "attach bootstrap: new-window dispatch failed",
                    );
                }
            }
        }
