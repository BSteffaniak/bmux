...
//!
//! The prompt **data types** ([`PromptRequest`], [`PromptField`],
//! [`PromptValidation`], etc.) are defined in [`bmux_plugin_sdk::prompt`]
//! and re-exported here for convenience.  This module adds the host
//! registration channel that connects prompt callers to the attach loop.
...
// ── Host registration (tokio-dependent) ──────────────────────────────────────
