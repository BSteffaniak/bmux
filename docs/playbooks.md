...
| `assert-state`     | Assert fake state; currently supports `windows.names`, `windows.active_name`, `scrollback.active`, `scrollback.cursor`, `selection.active`, `selection.text`, `help_overlay.open`, and `prompt.active` |
