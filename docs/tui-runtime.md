...
Applications construct the canonical component tree and retain its caller-owned state and derived layout caches. The presentation callback resolves that tree under the current terminal constraints, then paints it into the provided frame through `PaintCx`. The runtime schedules and commits this work; it does not own component measurement, choose application layout, or retain a second geometry tree. Input routing uses the exact successfully presented layout and interaction/selection scenes. A failed presentation must not publish the attempted layout, hit map, selection scene, focus geometry, image scene, or cursor as the source for subsequent input.

During migration, frame-level presenter callbacks may host both component and legacy surfaces, but this is a temporary consumer boundary rather than a second component lifecycle. The architectural endpoint has applications enter rendering through constraint-driven component layout and scoped paint. Runtime APIs must not recreate area-assigned widget layout, line scrolling, or product-specific viewport correction.
