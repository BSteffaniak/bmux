...
| --- | --- | --- |
| `StatusBarConfig` | `packages/config/src/lib.rs`; consumed by `packages/cli/src/status.rs`, `packages/cli/src/runtime/attach/runtime.rs`, and attach simulation | Move tab-strip settings to the presentation plugin; separately assign remaining status modules before removing the legacy configuration. |
| `StatusPosition` | `packages/config/src/lib.rs`; attach runtime/state/simulation and playbook engine | Replace top/bottom status placement with generic plugin layout placement and retain compatibility handling for supported legacy settings. |
