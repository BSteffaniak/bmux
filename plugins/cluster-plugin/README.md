...
- `cluster events` shows connection lifecycle events (`--format text|json`, `--cluster`, `--target`, `--state`, `--since`, `--limit`); `--since` accepts unix ms or relative durations like `15m`, `2h`, `1h30m`
